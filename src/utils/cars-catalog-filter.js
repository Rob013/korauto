/**
 * Cars Catalog Filter Utilities (JavaScript Version)
 * 
 * This module contains all the filtering logic for the car catalog.
 * It provides utilities for grade extraction, filter application,
 * filter validation, and search functionality.
 * 
 * JavaScript version of the TypeScript catalog-filter utilities.
 */

/**
 * Extracts grade information from a car title using various patterns
 * @param {string} title - The car title to extract grades from
 * @returns {string[]} Array of extracted grades
 */
export const extractGradesFromTitle = (title) => {
  const grades = [];
  const patterns = [
    /\b(\d+\.?\d*\s?(?:TDI|TFSI|FSI|TSI|CDI|T|D|I|E|H))\b/gi, // Include all engine types
    /\b(\d+\.?\d*)\s*l?i?t?e?r?\s*(?:TDI|TFSI|FSI|TSI|CDI|T|D|I|E|H)\b/gi,
    /\b(\d+\.?\d*[iIdDeEhH])\b/gi, // Specific patterns for all engine types: 520i, 530d, 530e, etc.
    /\b(\d+\.?\d*)\s*(?:hybrid|electric|diesel|petrol|gasoline)\b/gi, // Full word variants
  ];
  
  patterns.forEach(pattern => {
    const matches = title.match(pattern);
    if (matches) {
      matches.forEach(match => {
        const cleaned = match.trim();
        if (cleaned && !grades.includes(cleaned)) {
          grades.push(cleaned);
        }
      });
    }
  });
  
  return grades;
};

/**
 * Extracts all grades from a car object (from lots, title, and engine)
 * @param {Object} car - The car object
 * @returns {string[]} Array of extracted grades
 */
export const extractCarGrades = (car) => {
  const carGrades = [];
  
  // Extract grades from lots (primary source)
  if (car.lots && Array.isArray(car.lots)) {
    car.lots.forEach((lot) => {
      if (lot.grade_iaai) {
        carGrades.push(lot.grade_iaai.trim().toLowerCase());
      }
    });
  }
  
  // Extract grades from title
  if (car.title) {
    const titleGrades = extractGradesFromTitle(car.title);
    carGrades.push(...titleGrades.map(g => g.toLowerCase()));
  }
  
  // Extract grades from engine field
  if (car.engine && car.engine.name) {
    carGrades.push(car.engine.name.trim().toLowerCase());
  }
  
  return carGrades;
};

/**
 * Checks if a car matches a grade filter
 * @param {Object} car - The car object
 * @param {string} gradeFilter - The grade filter to match against
 * @returns {boolean} Whether the car matches the grade filter
 */
export const matchesGradeFilter = (car, gradeFilter) => {
  if (!gradeFilter || gradeFilter === 'all') {
    return true;
  }

  const filterGrade = gradeFilter.toLowerCase().trim();
  const carGrades = extractCarGrades(car);
  
  // More comprehensive matching for grades
  return carGrades.some(grade => {
    // Exact match
    if (grade === filterGrade) return true;
    
    // Partial match - both directions
    if (grade.includes(filterGrade) || filterGrade.includes(grade)) return true;
    
    // Remove spaces and try again
    const gradeNoSpaces = grade.replace(/\s+/g, '');
    const filterNoSpaces = filterGrade.replace(/\s+/g, '');
    if (gradeNoSpaces === filterNoSpaces) return true;
    
    // Handle special cases like "30 TDI" vs "30"
    const gradeParts = grade.split(/\s+/);
    const filterParts = filterGrade.split(/\s+/);
    if (gradeParts.some(part => filterParts.includes(part))) return true;
    
    return false;
  });
};

/**
 * Applies client-side grade filtering to a list of cars
 * @param {Object[]} cars - Array of car objects
 * @param {string} [gradeFilter] - The grade filter to apply
 * @returns {Object[]} Filtered array of cars
 */
export const applyGradeFilter = (cars, gradeFilter) => {
  if (!gradeFilter || gradeFilter === 'all') {
    return cars;
  }

  return cars.filter(car => matchesGradeFilter(car, gradeFilter));
};

/**
 * Validates and normalizes filter values
 * @param {Object} filters - The filters object
 * @returns {Object} Normalized filters object
 */
export const normalizeFilters = (filters) => {
  const normalized = {};
  
  Object.entries(filters).forEach(([key, value]) => {
    if (value !== undefined && value !== '' && value !== 'all' && value !== 'any') {
      normalized[key] = value;
    }
  });
  
  return normalized;
};

/**
 * Creates URL search parameters from filters
 * @param {Object} filters - The filters object
 * @returns {URLSearchParams} URL search parameters
 */
export const filtersToURLParams = (filters) => {
  const params = new URLSearchParams();
  const normalizedFilters = normalizeFilters(filters);
  
  Object.entries(normalizedFilters).forEach(([key, value]) => {
    if (value !== undefined && value !== '') {
      // Properly encode grade filter for URL
      params.set(key, key === 'grade_iaai' ? encodeURIComponent(value) : value);
    }
  });
  
  return params;
};

/**
 * Parses URL search parameters into filters
 * @param {URLSearchParams} searchParams - URL search parameters
 * @returns {Object} Filters object
 */
export const urlParamsToFilters = (searchParams) => {
  const filters = {};
  
  searchParams.forEach((value, key) => {
    if (value && value !== 'undefined') {
      // Properly decode grade filter from URL
      filters[key] = key === 'grade_iaai' ? decodeURIComponent(value) : value;
    }
  });
  
  return filters;
};

/**
 * Checks if any filters are currently active
 * @param {Object} filters - The filters object
 * @returns {boolean} Whether any filters are active
 */
export const hasActiveFilters = (filters) => {
  const normalizedFilters = normalizeFilters(filters);
  return Object.keys(normalizedFilters).length > 0;
};

/**
 * Checks if this is a year range filter change
 * @param {Object} newFilters - New filters object
 * @param {Object} currentFilters - Current filters object
 * @returns {boolean} Whether this is a year range change
 */
export const isYearRangeChange = (newFilters, currentFilters) => {
  return (
    newFilters.from_year !== currentFilters.from_year || 
    newFilters.to_year !== currentFilters.to_year
  ) && (newFilters.from_year || newFilters.to_year);
};

/**
 * Creates filters with pagination
 * @param {Object} filters - The filters object
 * @param {number} [perPage=50] - Number of items per page
 * @returns {Object} Filters with pagination
 */
export const addPaginationToFilters = (filters, perPage = 50) => {
  return {
    ...filters,
    per_page: perPage.toString()
  };
};

/**
 * Fallback grades for manufacturers when API data is not available
 * @param {string} manufacturerId - The manufacturer ID
 * @returns {Array<{value: string, label: string}>} Array of grade options
 */
export const getFallbackGrades = (manufacturerId) => {
  const fallbacks = {
    '9': ['320d', '320i', '325d', '330d', '330i', '335d', '335i', 'M3', 'M5', 'X3', 'X5'], // BMW
    '16': ['220d', '250', '300', '350', '400', '450', '500', 'AMG'], // Mercedes-Benz
    '1': ['30 TDI', '35 TDI', '40 TDI', '45 TDI', '50 TDI', '55 TFSI', '30 TFSI', '35 TFSI', '40 TFSI', '45 TFSI', '30', '35', '40', '45', '50', '55', 'RS', 'S'], // Audi
    '147': ['1.4 TSI', '1.6 TDI', '1.8 TSI', '2.0 TDI', '2.0 TSI', 'GTI', 'R'], // Volkswagen
    '148': ['2.0T', '2.5L', '1.8L Hybrid', 'Electric Motor', 'Elite', 'Premium', 'Sport', 'Comfort'], // Generic
    '2': ['Civic', 'Accord', 'CR-V', 'HR-V'], // Honda
    '3': ['Corolla', 'Camry', 'RAV4', 'Highlander'], // Toyota
    '4': ['Altima', 'Maxima', 'Rogue', 'Murano'], // Nissan
    '5': ['Focus', 'Fiesta', 'Mondeo', 'Kuga'], // Ford
    '6': ['Cruze', 'Malibu', 'Equinox', 'Tahoe'], // Chevrolet
  };
  
  return (fallbacks[manufacturerId] || []).map(grade => ({ value: grade, label: grade }));
};

/**
 * Manufacturer categories for sorting
 * @param {string} manufacturerName - The manufacturer name
 * @returns {{priority: number, categoryName: string}} Category information
 */
export const getManufacturerCategory = (manufacturerName) => {
  const categories = {
    german: {
      name: 'German Brands',
      brands: ['BMW', 'Mercedes-Benz', 'Audi', 'Volkswagen', 'Porsche', 'Opel'],
      priority: 1
    },
    korean: {
      name: 'Korean Brands', 
      brands: ['Hyundai', 'Kia', 'Genesis'],
      priority: 2
    },
    japanese: {
      name: 'Japanese Brands',
      brands: ['Toyota', 'Honda', 'Nissan', 'Mazda', 'Subaru', 'Lexus', 'Infiniti', 'Acura', 'Mitsubishi'],
      priority: 3
    },
    american: {
      name: 'American Brands',
      brands: ['Ford', 'Chevrolet', 'Cadillac', 'GMC', 'Tesla', 'Chrysler', 'Jeep', 'Dodge'],
      priority: 4
    },
    luxury: {
      name: 'Luxury/European Brands',
      brands: ['Land Rover', 'Jaguar', 'Volvo', 'Ferrari', 'Lamborghini', 'Maserati', 'Bentley', 'Rolls-Royce', 'Aston Martin', 'McLaren', 'Mini'],
      priority: 5
    },
    french: {
      name: 'French Brands',
      brands: ['Peugeot', 'Renault', 'Citroën'],
      priority: 6
    },
    italian: {
      name: 'Italian Brands', 
      brands: ['Fiat', 'Alfa Romeo'],
      priority: 7
    },
    other: {
      name: 'Other Brands',
      brands: ['Skoda', 'Seat'],
      priority: 8
    }
  };

  const categoryEntry = Object.values(categories).find(category => 
    category.brands.includes(manufacturerName.trim())
  );

  return {
    priority: categoryEntry?.priority || 999,
    categoryName: categoryEntry?.name || 'Other Brands'
  };
};

/**
 * Sorts manufacturers by category and car count
 * @param {Array} manufacturers - Array of manufacturer objects
 * @returns {Array} Sorted array of manufacturers
 */
export const sortManufacturers = (manufacturers) => {
  return manufacturers
    .filter(m => {
      // Ensure manufacturer has valid data from API
      return m.id && 
             m.name && 
             typeof m.name === 'string' && 
             m.name.trim().length > 0 &&
             (m.cars_qty && m.cars_qty > 0);
    })
    .sort((a, b) => {
      const aName = a.name.trim();
      const bName = b.name.trim();
      
      // Find category for each manufacturer
      const aCategoryInfo = getManufacturerCategory(aName);
      const bCategoryInfo = getManufacturerCategory(bName);
      
      // Sort by category priority first
      if (aCategoryInfo.priority !== bCategoryInfo.priority) {
        return aCategoryInfo.priority - bCategoryInfo.priority;
      }
      
      // Within same category, sort by car count (descending)
      const aCount = a.cars_qty || 0;
      const bCount = b.cars_qty || 0;
      if (aCount !== bCount) {
        return bCount - aCount;
      }
      
      // Finally, alphabetical
      return aName.localeCompare(bName);
    });
};

/**
 * Generate year range for filters
 * @param {number} [currentYear] - Current year (defaults to current year)
 * @returns {number[]} Array of years
 */
export const generateYearRange = (currentYear) => {
  const year = currentYear || new Date().getFullYear();
  // Fixed: Enhanced year range - 30 years from current year to 1996, corrected calculation
  return Array.from({ length: 32 }, (_, i) => Math.max(year + 2 - i, 1996));
};

/**
 * Generate year range presets for quick filtering
 * @param {number} [currentYear] - Current year (defaults to current year)
 * @returns {Array} Array of year preset objects
 */
export const generateYearPresets = (currentYear) => {
  const year = currentYear || new Date().getFullYear();
  return [
    { label: '2022+', from: 2022, to: year + 2 },
    { label: '2020+', from: 2020, to: year + 2 },
    { label: '2018+', from: 2018, to: year + 2 },
    { label: '2015+', from: 2015, to: year + 2 },
    { label: '2010+', from: 2010, to: year + 2 },
    { label: '2005+', from: 2005, to: year + 2 },
    { label: '2000+', from: 2000, to: year + 2 },
  ];
};

/**
 * Checks if strict filtering mode should be enabled
 * @param {Object} filters - The filters object
 * @returns {boolean} Whether strict filtering mode should be enabled
 */
export const isStrictFilterMode = (filters) => {
  return !!(filters.manufacturer_id || filters.model_id || 
            filters.color || filters.fuel_type || filters.transmission || 
            filters.from_year || filters.to_year || filters.buy_now_price_from || 
            filters.buy_now_price_to || filters.odometer_from_km || filters.odometer_to_km ||
            filters.seats_count || filters.max_accidents || filters.grade_iaai || filters.search);
};

/**
 * Debounce utility function for filter operations
 * @param {Function} func - Function to debounce
 * @param {number} wait - Wait time in milliseconds
 * @returns {Function} Debounced function
 */
export const debounce = (func, wait) => {
  let timeout;
  return (...args) => {
    clearTimeout(timeout);
    timeout = setTimeout(() => func(...args), wait);
  };
};

// Default export with all functions
export default {
  extractGradesFromTitle,
  extractCarGrades,
  matchesGradeFilter,
  applyGradeFilter,
  normalizeFilters,
  filtersToURLParams,
  urlParamsToFilters,
  hasActiveFilters,
  isYearRangeChange,
  addPaginationToFilters,
  getFallbackGrades,
  getManufacturerCategory,
  sortManufacturers,
  generateYearRange,
  generateYearPresets,
  isStrictFilterMode,
  debounce,
};