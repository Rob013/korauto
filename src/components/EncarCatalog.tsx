...
const EncarCatalog = ({
  ...
  return (
    <div className="flex min-h-screen bg-background">
        ...
        <div className="flex-shrink-0 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex items-center justify-between px-4 py-2.5">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              className="h-8 px-2 text-xs hover:bg-primary-foreground/20 text-primary-foreground"
            >
              <span>Clear</span>
            </Button>
            <Button
      ...
      {showFilters && (
        <div
          className={`fixed inset-0 z-30 lg:hidden transition-opacity duration-300 ${isMobile ? 'bg-black/70 backdrop-blur-md' : 'bg-black/50 backdrop-blur-sm'}`}
          onClick={() => {
            if (isMobile) {
              setShowFilters(false);
              setHasExplicitlyClosed(true);
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 ml-auto flex-shrink-0">
                ...
                <div className="relative flex-shrink-0">
                  ...
                  <AdaptiveSelect
                    ...
                    placeholder="Sort"
                    className="w-36 sm:w-44 h-8 sm:h-9 text-xs sm:text-sm pl-6"
                  ...
                  return (
                    <div key={car.id} id={`car-${car.id}`} data-lot-id={`car-lot-${lotNumber}`}>
                      <LazyCarCard
                        ...
                        images={[...(lot?.images?.normal || []), ...(lot?.images?.big || [])].filter(Boolean)}
                        vin={car.vin}
                        ...
                        source={(car as any)?.domain?.name || (car as any)?.domain_name || (car as any)?.source_api}
                        viewMode={viewMode}
                      />
                    </div>
                  );
