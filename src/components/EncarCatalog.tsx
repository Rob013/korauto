...
const EncarCatalog = ({ highlightCarId }: EncarCatalogProps = {}) => {
  ...
  const handleFiltersChange = useCallback(async (newFilters: APIFilters) => {
    ...
    if (isYearChange) {
      ...
      // Apply other filters with backend sorting
      applyFiltersInternal(newFilters, sortBy);
    }
  }, [applyFiltersInternal, handleOptimizedYearFilter, filters, setCars, setFilters, setTotalCount, sortBy]);
  ...
  const fetchAllCarsForSorting = useCallback(async () => {
    console.log('🔄 Legacy function - backend sorting now handles this automatically');
  }, []);
    ...
    <div className="flex min-h-screen bg-background">
      ...
      {!isMobile && (
        <div 
          ...
          w-80 sm:w-80 lg:w-72 h-full flex-shrink-0 overflow-y-auto rounded-lg
          lg:shadow-none
        `}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <Button 
                  ...
                  className="lg:hidden flex items-center gap-1 h-8 px-2"
                >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <MobileFilterUX
                  ...
                  selectedFiltersCount={Object.values(filters).filter(Boolean).length}
                >
                  <EncarStyleFilter
                    filters={filters}
                    manufacturers={manufacturers.length > 0 ? manufacturers : createFallbackManufacturers()}
                ...
                <div className="relative">
                  ...
                  <AdaptiveSelect
                    ...
                    onValueChange={(value: SortOption) => {
                      ...
                      setCurrentPage(1);
                      console.log(`🔄 User selected sort: ${value}, triggering backend sort for ${totalCount} cars`);
                      // Note: Backend sorting is handled by useEffect that watches sortBy changes
                    }}
                    placeholder="Sort"
