      ...
      if (allCachedCars && allCachedCars.length > 0) {
        ...
        const cachedCarResults = allCachedCars.map((car: CachedCarData) => {
          ...
          return {
            ...
            mileage: car.mileage || (lot?.odometer?.km ? `${lot.odometer.km.toLocaleString('en-US')} km` : undefined),
            ...
            if (carData && Array.isArray(carData)) {
              ...
              apiResults = carData.map((car: Record<string, any>) => {
                ...
                return {
                  ...
                  image: lot?.images?.normal?.[0] || lot?.images?.big?.[0],
                  mileage: lot?.odometer?.km ? `${lot.odometer.km.toLocaleString('en-US')} km` : undefined,
                  fuel: car.fuel?.name,
