-- Add price_cents and rank_score columns to cars_cache if they don't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'cars_cache' AND column_name = 'price_cents') THEN
    ALTER TABLE cars_cache ADD COLUMN price_cents BIGINT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'cars_cache' AND column_name = 'rank_score') THEN
    ALTER TABLE cars_cache ADD COLUMN rank_score REAL;
  END IF;
END $$;

-- Update existing records to calculate price_cents and rank_score
UPDATE cars_cache 
SET 
  price_cents = CASE 
  ...
  END,
  rank_score = CASE 
    WHEN price IS NOT NULL AND year IS NOT NULL THEN 
      -- Simple ranking: newer cars with reasonable prices get higher scores
      ((2024 - year::INTEGER) * -10) + (CASE WHEN price < 50000 THEN 50 ELSE 0 END) + RANDOM() * 100
    ELSE 
      RANDOM() * 100 
  END
WHERE price_cents IS NULL OR rank_score IS NULL;

-- Create function to get filtered car count
...
BEGIN
  ...
  -- Add filters if provided (same as count function)
  IF p_filters IS NOT NULL AND p_filters != '{}' THEN
    -- Make filter
    IF p_filters ? 'make' AND (p_filters->>'make') != '' THEN
      where_clause := where_clause || ' AND LOWER(c.make) = LOWER(' || quote_literal(p_filters->>'make') || ')';
