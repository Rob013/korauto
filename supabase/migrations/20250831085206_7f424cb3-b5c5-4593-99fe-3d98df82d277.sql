...
ADD COLUMN IF NOT EXISTS last_api_sync TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- Update cars table price_cents from price if needed
UPDATE public.cars 
SET price_cents = (price * 100)::BIGINT 
WHERE price_cents IS NULL AND price IS NOT NULL;

-- Create optimized indexes for better sync performance
CREATE INDEX IF NOT EXISTS idx_cars_cache_external_id ON public.cars_cache(external_id);
...
-- Create function to handle sync completion tracking
...
BEGIN
  ...
  IF sync_status.status = 'running' THEN
    -- Resume from current page
    last_page := COALESCE(sync_status.current_page, 0);
